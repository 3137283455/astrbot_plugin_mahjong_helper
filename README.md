...
- 战绩：牌谱屋基本、顺位、立直、更多、和铳、血统、大铳、趋势和同桌分栏；支持四麻/三麻与金、玉、王座筛选。
- 对局：最近牌谱链接与名次结果。
- 订阅：按群或私聊会话自动播报新对局。
...
- `/雀魂切换 UID`、`/雀魂解绑 [UID]`
- `/雀魂`：查看分栏菜单和参数用法。
- `/雀魂 基本 [UID或昵称] [三麻] [金|玉|王座]`：和牌、放铳、打点等基本数据。
- `/雀魂 顺位|立直|更多|和铳|血统|大铳 [玩家] [三麻] [房间]`：牌谱屋对应栏目。
- `/雀魂 趋势 [玩家] [三麻] [房间]`：近 20 场顺位和段位分。
- `/雀魂 同桌 [玩家] [三麻] [房间]`：近 100 场最常同桌玩家。
- `/雀魂 对局 [玩家] [三麻] [房间] [页码]`：每页 5 场；页码 1～20。
- `/雀魂查询 [UID或昵称] [金|玉|王座]`
- `/查询三麻 [UID或昵称] [金|玉|王座]`
- `/雀魂对局 [UID或昵称] [金|玉|王座]`
- `/三麻对局 [UID或昵称] [金|玉|王座]`

不填写玩家时使用发送者的主绑定账号。分栏命令默认四麻、全部房间、全时间；
可加 `近7天`、`近30天`、`近90天` 或 `近365天`，参数顺序可交换。例如
`/雀魂 立直 12105509 四麻 玉 近30天` 或 `/雀魂 对局 12105509 三麻 2`。
