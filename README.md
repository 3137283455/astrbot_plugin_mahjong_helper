...
雀魂搜索、战绩和订阅通过牌谱屋公开接口读取，默认不需要 Token。牌谱屋只收录
金之间及以上的公开段位场数据，因此低段位玩家或未参加对应场次的玩家可能搜不到。

`koromo_token` 保留为可选兼容项。如果使用的镜像以后要求 Token，可以在 AstrBot
配置页填写，或由管理员**私聊**机器人：
...
不要在群聊中发送 Token。何切功能不依赖任何外部接口，题库已经内置，无需另外整理。
...
- 何切重置、每日自动出题、订阅管理、设置可选牌谱屋 Token、本地雀魂登录和取谱测试
  只允许 AstrBot 管理员执行。
- 可选牌谱屋 Token、雀魂账号和密码只能在私聊中提交；插件不会把雀魂登录密码写入
  插件数据库。
