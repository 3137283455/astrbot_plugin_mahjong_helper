...
`搜索歌曲 晴天` 后发送 `/歌单 加 1`，即可把搜索结果第 1 首加入歌单；播放过一首歌后，也可以直接发送 `/歌单 加`。使用 `/歌单` 查看列表，`/歌单 播 1` 直接播放第 1 首，`/歌单 删 1` 删除。歌单默认每人独立，插件配置中的“歌单归属”可切换为群共享。歌单保存在 AstrBot 插件数据目录，重启后仍在；保存的是 B 站视频和分 P 标识，播放时会重新取得媒体地址，但不会重新搜索或让用户选歌。
