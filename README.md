...
`/雀 桌` 和对局订阅在接口触发验证时不可用。尚未找到公开的固定 API 密钥申请入口；
...
牌谱屋目前对机器人访问对局列表返回 `x-cap-token-required`。因此“对局、趋势、同桌”
以及旧版最近对局命令会提示接口受限；“基本、顺位、立直、更多、和铳、血统、大铳”
仍可从公开统计接口查询。直接抓取网页也会调用同一个受限接口，不能稳定解决订阅。
